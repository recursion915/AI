...
public class LikelihoodWeighting {
	...
	/*
	 * Inner class WeightedAssignment
	 * this object is used to return an assignment and a weight(double) value 
	 */
	public class WeightedAssignment{
		...
		public WeightedAssignment(){
			
		}
		public WeightedAssignment(Assignment x, double weight){
			this.x = x;
			this.weight = weight;
		}
		...
		public void setAssignment(Assignment x){
			this.x = x;
		}
	...
	public Distribution ask(BayesianNetwork bn, RandomVariable X, Assignment e, int numberArguments) {
		WeightedAssignment weightedAss = new WeightedAssignment();
		Assignment tempAss = new Assignment();
		Distribution distribution = new Distribution();
		Assignment duplicatedEvidence = e.copy();
		...
		double [] count = new double[X.getDomain().size()];
	
		// loop through request times
		for(int i = 0; i< numberArguments; i++){
				// get the randomGenerated assignment
				weightedAss = weightedSample(sortedRVList);
				tempAss = weightedAss.getAssignment();
		//		System.out.println("generated random sample is "+tempAss);
				for(int j = 0; j<X.getDomain().size(); j++){
				// add the query domain into evidence
					duplicatedEvidence.set(X, X.getDomain().get(j));
				// if generated Assignment satisfies with evidence
				// count++
					if(isConsistent(tempAss, duplicatedEvidence)){
						count[j] = count[j] + weightedAss.getWeight();
					}
				}
		}
...
//		System.out.println("Assignment contains evidence is "+ assList.containsAll(eList));
	 ...
	 */
	public WeightedAssignment weightedSample(List <RandomVariable> rvList){
		...
		for (int i = 0; i< rvList.size(); i++){
			...
			// if this random variable is in evidence List
			// set this random variable to evidence domain
			// then weight times probably
			if(evidence.containsKey(tempRV)){
				tempEvidence.set(tempRV, evidence.get(tempRV));
//				System.out.println("old weight "+ weight + "new prob" + network.getProb(tempRV, tempEvidence));
				weight = weight * network.getProb(tempRV, tempEvidence);
//				System.out.println("evidence contains " + tempRV + "prob: " + network.getProb(tempRV, tempEvidence) + 
//						"real evidence is " + evidence);
			}
			// otherwise, random generate a domain
			else{
				tempEvidence.set(tempRV, generateRandomAssignment(tempRV,tempEvidence));
//				System.out.println(tempEvidence.toString());
//				System.out.println("evidence not contains " + tempRV + "prob: " + network.getProb(tempRV, tempEvidence));
			}
			
		}
		
		WeightedAssignment generatedEvidence = new WeightedAssignment(tempEvidence, weight);
...
//		test.weightedSample(test.sortedRVList);
